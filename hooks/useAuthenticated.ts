...
import { getAccessToken, getRefreshToken, isAuthenticated, setAccessToken } from "../utils/spotify";
...
export default function useAuthenticated(): boolean {
  ...
  useEffect(() => {
    ...
    if (!authenticated && authToken && refreshToken) {
      axios.get(`api/refresh_token?refresh_token=${refreshToken}`).then((res) => {
        const { data } = res;
